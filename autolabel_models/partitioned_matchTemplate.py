...
def partitioned_matchTemplate_v2(
        image,
        templ,
        method,
        threshold,
    ...
    """
    cv2.matchTemplate, but operated using
    partitioned image, and return boxes and scores.
    Very memory efficient, depends on `part_shapelim`
    and `threshold`.
    ...
    method
        ...
        see TemplateMatchModes
    threshold
        Threshold used to filter out low matching probability
        results. For the TM_SQDIFF and MT_SQDIFF_NORMED methods,
        the best match are the lowest scores, so filter out
        scores higher than threshold. For all the others,
        higher values represent better matches, so filter out
        scores lower than threshold.
    mask
        Optional mask. It must have the same size as templ.
        It must either have the same number of channels as
        template or only one channel, which is then used for
    ...
    tmp_res_h, tmp_res_w = ps_h, ps_w - templ_w + 1
    ...
    for j_y in range(y_parts_cnt):
        for i_x in range(x_parts_cnt):
            ...
            _bboxes, _scores = match_result_to_bboxes(
                ...
                x_offset=x_res_s,
                y_offset=y_res_s)
            bboxes_concat.append(_bboxes)
            scores_concat.append(_scores)
    ...
    if not bboxes_concat:
        bboxes_concat = np.empty((0, 4), np.int32)
        scores_concat = np.empty((0,), dtype=np.float32)
    else:
        bboxes_concat = np.concatenate(bboxes_concat, axis=0)
        scores_concat = np.concatenate(scores_concat, axis=0)
    return bboxes_concat, scores_concat
