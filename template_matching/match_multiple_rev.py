...
ap.add_argument("-i", "--images", help="Path to images where template will be matched")
...
def templateMatching(i, imageUrl):
	...
	for scale in scales:
		...
		for templateUrl in list(args['templates'].split(",")):
			...
			_, template = cv2.threshold(template, 150, 255, 
										cv2.THRESH_BINARY+cv2.THRESH_OTSU)
			tW, tH = template.shape[::-1]

			# determine the bounding box parameters
			valve_type = templateUrl[templateUrl.find("templates/")+10:templateUrl.find("lwKuT")]
			threshold = 0.6
			...
			res = partitioned_matchTemplate(resized, template, cv2.TM_CCOEFF_NORMED)
			loc = np.where(res >= threshold)
			
			# only include bounding boxes that are not overlapping
			for pt in zip(*loc[::-1]):
				score = res[pt[1], pt[0]]
				bb1 = (int(pt[0] * r), int(pt[1] * r), 
					int((pt[0] + tW) * r), int((pt[1] + tH) * r))
